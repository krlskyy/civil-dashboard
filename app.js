        ...
        const ICONS = {
            ...
            barChart: (s=14) => svgWrap(s, `<line x1="12" x2="12" y1="20" y2="10"/><line x1="18" x2="18" y1="20" y2="4"/><line x1="6" x2="6" y1="20" y2="16"/>`)
        };
        const icon = (name, s=16) => `<span class="icon-svg" style="width:${s}px; height:${s}px;">${ICONS[name] ? ICONS[name](s) : ICONS.fileText(s)}</span>`;
        ...
        document.getElementById('sidebar-nav').innerHTML = `
            ...
            <button class="nav-link admin-link" data-tab="results-tab" style="display:none;" onclick="switchTab('results-tab')">${icon('fileText', 16)}<span>Итоги</span></button>
        ...
        document.getElementById('ct-results-manage').innerHTML = `${icon('barChart', 14)}<span>Управление итогами</span>`; 
        document.getElementById('new-result-btn').innerHTML = `${icon('plus', 14)}<span>Новая неделя</span>`;
        ...
        document.getElementById('bottom-nav').innerHTML = `
            ...
            <button class="bottom-nav-btn admin-link" data-tab="results-tab" style="display:none;" onclick="switchTab('results-tab')"><span class="icon">${icon('fileText', 18)}</span>Итоги</button>
        ...
        function switchTab(tabId) {
            ...
            else if (tabId === 'results-tab') loadWeeklyResults();
        ...
        /* ========== УПРАВЛЕНИЕ ИТОГАМИ (CIVIL RESULTS) ========== */
        
        let resultState = { workspace: [], gameplay: [], assistants: [] };
        let isEditingResult = false;

        async function loadWeeklyResults() {
            if (!currentUserData || !['head', 'deputy_head'].includes(currentUserData.role)) return;
            const container = document.getElementById('results-admin-list');
            container.innerHTML = `<p style="text-align: center; color: var(--text-muted); padding: 20px 0;">Загрузка...</p>`;
            ...
            if (error) {
                container.innerHTML = `<p style="color:var(--danger); font-size:12px; text-align:center;">Ошибка: ${error.message}</p>`;
                return;
            }

            if (!data || data.length === 0) {
                container.innerHTML = `<div class="empty-state"><div class="empty-desc">Нет сохраненных итогов</div></div>`;
                return;
            }

            container.innerHTML = data.map(r => `
                <div class="admin-row" style="align-items:center; display:flex;">
                    <div style="flex:1;">
                        <div class="admin-row-name">${escapeHtml(r.id)} <span style="color:var(--text-muted); font-weight:400; font-size:11px;">— ${escapeHtml(r.period_label)}</span></div>
                        ${r.is_current ? `<span class="points-badge" style="background:var(--role-manager-soft); color:var(--role-manager);">Актуальные</span>` : ''}
                    </div>
                    <div class="admin-row-controls">
                        <button class="view-row-btn" onclick="editWeeklyResult('${r.id}')">${icon('settings', 12)}<span>Ред.</span></button>
                        <button class="delete-row-btn" onclick="deleteWeeklyResult('${r.id}')">${icon('trash2', 12)}</button>
                    </div>
                </div>
            `).join('');
        }

        function toggleResultEditor(reset = true) {
            const editor = document.getElementById('result-editor');
            const opening = editor.style.display === 'none';
            editor.style.display = opening ? 'flex' : 'none';
            
            if (!opening && reset) {
                document.getElementById('res-id').disabled = false;
                document.getElementById('res-id').value = '';
                document.getElementById('res-period').value = '';
                document.getElementById('res-rank').value = '';
                document.getElementById('res-budget').value = '0';
                document.getElementById('res-max').value = '0';
                document.getElementById('res-summary').value = '';
                document.getElementById('res-pers-text').value = '';
                document.getElementById('res-pers-note').value = '';
                document.getElementById('res-is-current').checked = false;
                resultState = { workspace: [], gameplay: [], assistants: [] };
                renderResultDynamics();
                isEditingResult = false;
            }
        }

        async function editWeeklyResult(id) {
            const { data, error } = await supabaseClient.from('weekly_results').select('*').eq('id', id).single();
            if (error) return showToast('Ошибка загрузки: ' + error.message, true);
            
            toggleResultEditor(false);
            isEditingResult = true;
            document.getElementById('res-id').value = data.id;
            document.getElementById('res-id').disabled = true;
            document.getElementById('res-period').value = data.period_label || '';
            document.getElementById('res-rank').value = data.rank_among_segments || '';
            document.getElementById('res-budget').value = data.segment_budget || 0;
            document.getElementById('res-max').value = data.max_per_assistant || 0;
            document.getElementById('res-summary').value = data.summary || '';
            document.getElementById('res-pers-text').value = data.personnel_text || '';
            document.getElementById('res-pers-note').value = data.personnel_note || '';
            document.getElementById('res-is-current').checked = data.is_current;

            resultState.workspace = Array.isArray(data.workspace_stats) ? data.workspace_stats : [];
            resultState.gameplay = Array.isArray(data.gameplay_points) ? data.gameplay_points : [];
            resultState.assistants = Array.isArray(data.assistants) ? data.assistants : [];
            
            renderResultDynamics();
        }

        function renderResultDynamics() {
            // Workspace
            document.getElementById('dyn-workspace-list').innerHTML = resultState.workspace.map((item, i) => `
                <div style="display:flex; gap:6px;">
                    <input type="text" class="form-control" placeholder="Значение (напр. 100% или ✓)" value="${escapeHtml(item.value)}" onchange="resultState.workspace[${i}].value=this.value" style="flex:0.4;">
                    <input type="text" class="form-control" placeholder="Лейбл (напр. регистрация)" value="${escapeHtml(item.label)}" onchange="resultState.workspace[${i}].label=this.value" style="flex:1;">
                    <button class="delete-row-btn" onclick="resultState.workspace.splice(${i}, 1); renderResultDynamics();" style="padding:6px 10px;">${icon('x', 12)}</button>
                </div>
            `).join('');

            // Gameplay
            document.getElementById('dyn-gameplay-list').innerHTML = resultState.gameplay.map((item, i) => `
                <div style="display:flex; gap:6px;">
                    <textarea class="form-control" rows="1" placeholder="Текст пункта..." onchange="resultState.gameplay[${i}]=this.value" style="flex:1; resize:vertical;">${escapeHtml(item)}</textarea>
                    <button class="delete-row-btn" onclick="resultState.gameplay.splice(${i}, 1); renderResultDynamics();" style="padding:6px 10px; height: fit-content;">${icon('x', 12)}</button>
                </div>
            `).join('');

            // Assistants
            document.getElementById('dyn-assistants-list').innerHTML = resultState.assistants.map((ast, i) => `
                <div style="background: var(--bg-surface-raised); padding: 10px; border-radius: var(--radius-sm); border: 1px solid var(--border-soft);">
                    <div class="task-editor-row" style="margin-bottom:6px;">
                        <input type="text" class="form-control" placeholder="Имя (напр. achikatomuro)" value="${escapeHtml(ast.name)}" onchange="resultState.assistants[${i}].name=this.value" style="flex:1;">
                        <select class="form-control" onchange="resultState.assistants[${i}].rank=this.value" style="flex:1;">
                            <option value="">Без плашки</option>
                            <option value="gold" ${ast.rank==='gold'?'selected':''}>Gold (наибольшее BWC)</option>
                            <option value="silver" ${ast.rank==='silver'?'selected':''}>Silver (среднее BWC)</option>
                            <option value="bronze" ${ast.rank==='bronze'?'selected':''}>Bronze (меньшее BWC)</option>
                        </select>
                        <input type="number" class="form-control" placeholder="BWC" value="${ast.bwc || 0}" onchange="resultState.assistants[${i}].bwc=parseInt(this.value)||0" style="flex:0.5;">
                    </div>
                    <div style="display:flex; gap:6px;">
                        <textarea class="form-control" rows="2" placeholder="Комментарий / сводка..." onchange="resultState.assistants[${i}].text=this.value" style="flex:1; resize:vertical;">${escapeHtml(ast.text)}</textarea>
                        <button class="delete-row-btn" onclick="resultState.assistants.splice(${i}, 1); renderResultDynamics();" style="padding:6px 10px; height: fit-content;">${icon('trash2', 14)}</button>
                    </div>
                </div>
            `).join('');
        }

        function addResultStat() { resultState.workspace.push({value:'', label:''}); renderResultDynamics(); }
        function addResultGameplay() { resultState.gameplay.push(''); renderResultDynamics(); }
        function addResultAssistant() { resultState.assistants.push({name:'', rank:'', text:'', bwc:0}); renderResultDynamics(); }
        ...
        async function saveWeeklyResult() {
            ...
            if (!id) return showToast('Укажи ID недели (YYYY-MM-DD)', true);
            ...
            const payload = {
                id: id,
                period_label: document.getElementById('res-period').value.trim(),
                rank_among_segments: document.getElementById('res-rank').value.trim() || null,
                segment_budget: parseInt(document.getElementById('res-budget').value) || 0,
                max_per_assistant: parseInt(document.getElementById('res-max').value) || 0,
                summary: document.getElementById('res-summary').value.trim(),
                personnel_text: document.getElementById('res-pers-text').value.trim() || null,
                personnel_note: document.getElementById('res-pers-note').value.trim() || null,
                workspace_stats: resultState.workspace.filter(i => i.value || i.label),
                gameplay_points: resultState.gameplay.filter(i => i.trim()),
                assistants: resultState.assistants.filter(i => i.name),
                is_current: document.getElementById('res-is-current').checked,
                created_by: currentUserData.id
            };

            try {
                if (isEditingResult) {
                    const { error } = await supabaseClient.from('weekly_results').update(payload).eq('id', id);
                    if (error) throw error;
                } else {
                    const { error } = await supabaseClient.from('weekly_results').insert([payload]);
                    if (error) throw error;
                }
                showToast('Итоги успешно сохранены');
                toggleResultEditor();
                loadWeeklyResults();
            } catch (err) {
                showToast('Ошибка: ' + err.message, true);
                ...
                btn.disabled = false;
                btn.innerText = 'Сохранить итоги';
            }
        }

        async function deleteWeeklyResult(id) {
            if (!confirm(`Удалить итоги недели ${id}? Это действие необратимо.`)) return;
            const { error } = await supabaseClient.from('weekly_results').delete().eq('id', id);
            if (error) return showToast('Ошибка: ' + error.message, true);
